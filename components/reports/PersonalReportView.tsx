...
import { columnHeader, radialDy, radialTextAnchor, REPORT_TYPE } from '@/lib/report-export/typography';
...
function RadarChart({ data, theme }: { data: PersonalReportData; theme: typeof REPORT_THEMES.personal }) {
  ...
  const labelPad = 72;
  const core = 300;
  const viewSize = core + labelPad * 2;
  const cx = viewSize / 2;
  const cy = viewSize / 2;
  const r = 108;
  const n = cats.length;
  ...
  return (
    <svg
      ...
      style={{ display: 'block', margin: '0 auto', maxWidth: 480, overflow: 'visible' }}
    ...
    >
      ...
      {cats.map((c, i) => {
        const a = angle(i);
        const lx = cx + (r + 50) * Math.cos(a);
        const ly = cy + (r + 50) * Math.sin(a);
        return (
  ...
  return (
    ...
    >
      ...
      <SectionBlock>
        ...
        >
          <ReportPanel theme={theme} title="部門別得点" style={{ gridRow: 'span 2' }}>
                ...
                {data.analysis.categoryScores
                  ...
                  .map((c, i) => (
                    <ReportTr key={c.key} theme={theme} index={i}>
                      ...
                      <ReportTd theme={theme} align="right">{c.earnedScore}</ReportTd>
      ...
      <SectionBlock style={{ marginBottom: 0 }}>
        ...
        <p style={{ marginTop: 10, fontSize: REPORT_TYPE.caption, color: theme.inkMuted }}>
          ○ 正解 · △ 一部一致 · × 不正解 · — 未判定
        </p>
      </SectionBlock>
    </ReportShell>
