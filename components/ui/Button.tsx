...
export function Button({ variant = 'primary', children, className = '', disabled, ...props }: ButtonProps) {
  ...
  const variantClasses = {
    primary: disabled
      ? 'bg-bd-accent/35 text-bd-accent-foreground/70 cursor-not-allowed border border-bd-accent/20'
    ...
    secondary: disabled
      ? 'bg-transparent text-stone-400/70 cursor-not-allowed border border-white/12'
